...
export interface RepositoryScene {
   ...
   *
   * Path conventions for the entire view model, frozen by `docs/contract.md`:
   * - Every path is prefixed with the repository folder name. The root is
   *   `8-bit-FM`, its children `8-bit-FM/src`, and so on.
   * - The join rule has no special case for the root:
   ...
   * - Ancestor tests must append the separator (`path.startsWith(dir.path + "/")`),
   *   or `8-bit-FM/src-tauri` reads as a descendant of `8-bit-FM/src`.
   */
...
export interface SceneFolder {
  ...
  /**
   * The complete path to this folder as the backend emits it, repository-prefixed
   * (`8-bit-FM/src`) and forward-slashed on every operating system.
   *
   * This is the shared selection identity, frozen by `docs/contract.md`: the
   * Canvas scene and the accessible tree both identify a folder by this exact
   * string, and the store's `expanded` set and `selectedPath` are made of them.
   * It is passed through the adapter untouched — trimming the repository prefix
   * would break selection sync silently, with no error anywhere.
   */
